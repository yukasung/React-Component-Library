...
import { waitFor, act, cleanup, fireEvent, render, screen, within } from '@testing-library/react'
...
describe('InputDateTime', () => {
  ...
  it('preserves and updates local calendar theme without changing another calendar', async () => {
    const view = (dark: boolean, color: string) => <>
      <div className={dark ? 'dark' : ''}>
        <InputDateTime style={{ '--rc-color-primary': color, fontFamily: 'serif' } as React.CSSProperties} />
      </div>
      <InputDateTime />
    </>
  ...
  describe('on an iPhone', () => {
    ...
    it('commits an English native date selection after calendar prop updates', () => {
      ...
      const { rerender, unmount } = render(
        <StrictMode>
          <InputDateTime defaultValue={new Date(2026, 6, 1, 9, 30)} onChange={onChange} />
        </StrictMode>,
      )
      ...
      rerender(
        <StrictMode>
          <InputDateTime
            defaultValue={new Date(2026, 6, 1, 9, 30)}
            onChange={onChange}
            monthCount={2}
      ...
      expect(onChange).toHaveBeenCalledExactlyOnceWith(new Date(2026, 6, 15, 9, 30))
      expect(getInput()).toHaveValue('2026-07-15 09:30')
      unmount()
    ...
    it('keeps native selections Gregorian after switching to Thai locale', () => {
      const onChange = vi.fn()
      const { rerender } = render(<InputDateTime defaultValue={new Date(2026, 6, 1, 9, 30)} onChange={onChange} />)
      rerender(<InputDateTime defaultValue={new Date(2026, 6, 1, 9, 30)} onChange={onChange} locale="th" monthCount={2} />)
      ...
      expect(getInput()).toHaveValue('2569-07-01 09:30')

      fireEvent.change(nativeInput, { target: { value: '2026-07-15' } })

      expect(onChange).toHaveBeenCalledExactlyOnceWith(new Date(2026, 6, 15, 9, 30))
      expect(getInput()).toHaveValue('2569-07-15 09:30')
      expect(nativeInput).toHaveValue('2026-07-15')
    })

    it('keeps the Thai Buddhist Era JavaScript calendar on mobile', () => {
      render(<InputDateTime defaultValue={new Date(2026, 6, 1, 9, 30)} locale="th" />)
  ...
  describe('popup configuration', () => {
    ...
    it('takes its layer from the --rc-z-popup token', async () => {
      ...
      try {
        render(<InputDateTime value={new Date(2026, 6, 15, 9, 30)} onChange={() => {}} />, { container: host })
        await user.click(screen.getByRole('button', { name: 'Toggle time list' }))
    ...
    it('lets portalZIndex override the --rc-z-popup token', async () => {
      ...
      try {
        render(<InputDateTime value={new Date(2026, 6, 15, 9, 30)} onChange={() => {}} portalZIndex={7} />, { container: host })
        await user.click(screen.getByRole('button', { name: 'Toggle time list' }))
