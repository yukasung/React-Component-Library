...
Neither component drives the masker for ordinary typing any more — both edit through fixed-width groups (see the next section). `InputDate` still holds it for the alphabetic formats groups can't describe, where it does nothing but pass text through. The token-agnostic split above is what let the group editor reuse the segments and the character-acceptance rules rather than forking them.
...
- **`InputDate` keeps the masker too.** An alphabetic format (`F j, Y`) has no fixed-width shape, so `tokenizeDateMask` returns undefined and the field falls back to plain text editing through `useInputMask` (which then only passes text through). That fallback is also what an edit at an *unfocused* field takes. `InputTime`'s formats are all maskable, so it dropped `useInputMask` entirely. One consequence worth knowing: the masker's own engine (`applyInputMask`, the ambiguous-digit timeout) is no longer reachable from either component's real typing path — it is exercised by `inputMask.test.ts` alone.
...
- **Which side the zeros go on is decided by the range, not the token.** `padSlot` right-aligns a group that *has* a `min`/`max` — it holds a number that must land in that range, so a minute typed `3` is `03`. A group *without* one is a year (`Y`/`y` are range-less precisely because any digit is valid at any position) and is read most-significant-first, so it fills to the right: `2` shows `2000`, then `2020`, then `2026`. The committed string goes through the same function, so a group can never commit as something other than what it was showing (`2` commits as `2000`, never `0002`).
